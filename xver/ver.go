...
func Get() (*SysVer, error) {
	...
	if runtime.GOOS == "linux" {
		...
		if cutPos >= 0 {
			...
			tmp, err := xstring.SubstrAscii(sv.PlatformVer, cutPos+8, len(sv.PlatformVer)-1)
			if err == nil {
				sv.PlatformVer = tmp
			}
		}
		...
		return nil, errors.New(runtime.GOOS + " unsupported")
	}
	...
	if runtime.GOOS == "linux" {
		...
		if do == nil {
			return nil, errors.New("LinuxDistribution error")
		}
		...
		if spacePos > 0 {
			tmp, err := xstring.SubstrAscii(sv.LinuxDistroName, 0, spacePos)
			if err == nil {
...
func ForbiddenCentOs5x() {
	...
	if sv.LinuxDistroName == "centos" && len(sv.LinuxDistroVer) > 0 {
		majorVerString, err := xstring.SubstrAscii(sv.LinuxDistroVer, 0, 1)
		if err != nil {
			return
		}
		...
		if majorVerNum > 1 && majorVerNum <= 5 {
			xlog.Fata(errors.New(sv.LinuxDistroName + " " + sv.LinuxDistroVer + " unsupported"))
			os.Exit(-1)
		}
	}
}
