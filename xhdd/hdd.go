...
type VolumeInfo struct {
	...
	Available xvolume.Volume
	Free      xvolume.Volume
	Total     xvolume.Volume
}
...
func GetVolumeInfo(volumePath string) (*VolumeInfo, error) {
	...
	vi.Available, err = xvolume.FromByteSize(float64(du.Total))
	if err != nil {
		return nil, err
	}
	vi.Free, err = xvolume.FromByteSize(float64(du.Total))
	if err != nil {
		return nil, err
	}
	vi.Total, err = xvolume.FromByteSize(float64(du.Total))
	if err != nil {
		return nil, err
	}
